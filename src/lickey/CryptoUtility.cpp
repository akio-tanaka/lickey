...
namespace lickey
{
	...
	bool Decrypt(const unsigned char* data, const size_t datalen, const unsigned char* key, const unsigned char* iv,
	             unsigned char* dest, size_t& destlen)
	{
		EVP_CIPHER_CTX de;
		int f_len = 0;
		...
		EVP_DecryptFinal_ex(&de, static_cast<unsigned char*>(dest + static_cast<unsigned char>(p_len)), &f_len);
		...
		destlen = static_cast<size_t>(p_len) + static_cast<size_t>(f_len);
		...
		std::string& str)
	{
		...
		char* buf = static_cast<char*>(malloc(bptr->length));
		if (buf == nullptr)
		{
			assert(buf);
		}
